  ...
  return (
    <Box py={{ base: 5, md: 10 }}>
      <Container maxW="container.xl">
        <Grid templateColumns={{ base: "1fr", lg: "3fr 2fr" }} gap={{ base: 6, md: 12 }}>
          {/* Left Column - Event Details */}
          <VStack align="stretch" spacing={8}>
            {event.image ? (
              <Image src={event.image} alt={event.name} borderRadius="2xl" objectFit="contain" h={{ base: "200px", md: "350px" }} />
            ) : (
              <Box h={{ base: "200px", md: "350px" }} w="100%" display="flex" alignItems="center" justifyContent="center" bg="gray.600" borderRadius="2xl">
          ...
          {/* Right Column - Ticket Selection (only for active events) */}
          {event.isActive === 1 && (
            <VStack align="stretch" spacing={{ base: 2, md: 6 }}>
                ...
                <VStack align="stretch" bg="gray.600" p={6} borderRadius="xl" spacing={4}>
                  ...
                  {summary.selectedTickets.map((tier) => (
                    <Box key={tier.id}>
                      <Flex justify="space-between" align="center">
                        ...
                        <Text fontWeight="bold">{isFreeEvent ? "Gratis" : formatARS(tier.price * quantities[tier.id])}</Text>
                      </Flex>
                      {!isFreeEvent && (
                        <>
                  ...
                  <Flex justify="space-between">
                    ...
                    <Text fontSize="lg" fontWeight="bold" color="brand.400">
                      {isFreeEvent ? "Gratis" : formatARS(summary.total)}
                    </Text>
                  </Flex>
                </VStack>
              )}
              ...
              {summary.selectedTickets.length > 0 && (
                <Box mt={4} mb={2} bg="gray.700" borderRadius="xl" boxShadow="md" px={{ base: 4, md: 6 }} py={4} display="flex" flexDirection="column" gap={2}>
              ...
              <Button
                ...
                isLoading={isLoading}
                loadingText={isFreeEvent ? "Procesando reserva..." : "Procesando compra..."}
                onClick={isFreeEvent ? onFreePurchase : onPurchase}
              >
                {summary.selectedTickets.length === 0
              ...
              {summary.selectedTickets.length > 0 && !isFreeEvent && (
          ...
          {/* Right Column - Inactive Event */}
          {event.isActive === 0 && (
            <Box p={{ base: 4, md: 6 }} bg="gray.600" borderRadius="xl" h="max-content">
              <VStack spacing={2} align="center">
                <Text color="whiteAlpha.700" textAlign="center" fontSize="lg">
                  Evento Finalizado
                </Text>
                <Text color="whiteAlpha.600" textAlign="center" fontSize="sm">
                  La venta de entradas ya no está disponible
                </Text>
              </VStack>
            </Box>
          )}
